...
typedef struct my_metadata_t {
  size_t size;
  struct my_metadata_t *next; /*未使用のメモリへのポインタ*/
} my_metadata_t;
...
typedef struct my_heap_t {
  ...
  my_metadata_t dummy; /*先頭のダミー*/
} my_heap_t;

//
// Static variables (DO NOT ADD ANOTHER STATIC VARIABLES!)
//
my_heap_t my_heap[bin_size];
...
void my_add_to_free_list(my_metadata_t *metadata,int index) {
  ...
  // リストを探索
  bool merged = false;
  my_metadata_t *prev = my_heap[index].free_head;
...
void my_remove_from_free_list(my_metadata_t *metadata, my_metadata_t *prev,int index) {
  //int index = define_index(metadata->size);
  if (prev) {
...
void *my_malloc(size_t size) {
  ...
  if (remaining_size > sizeof(my_metadata_t)) {
    ...
    int remain_index = define_index(remaining_size);
    my_add_to_free_list(new_metadata,remain_index);
  }
  return ptr;
}
